...
const uint8_t ShtResponseBase::iCRCTable[256] = 
{
...
uint8_t ShtResponseBase::GetCRC( uint8_t const aMsg[], const uint32_t aBytesCount )
{
    uint8_t data;
    uint8_t crc = 0xFF;

    for ( uint32_t byte = 0; byte < aBytesCount; ++byte )
    {
        data = aMsg[byte] ^ crc;
        crc = iCRCTable[data] ^ (crc << 8);
    }
